...
def paired_test_analysis(higher_res,lower_res):
	...
	lower_odd = res_dict['even_%s'%(lower_res)]

	_,_,same_stim_relilability = utils.neuronal_CC_paired_stim(higher_odd,higher_even,neuron_pass=None,to_plot = False,save_dir = os.path.join(save_image,'same_res_%s_%s'%(higher_res,higher_res)),xlab='odd_%s'%(higher_res),ylab='even_%s'%(higher_res))
	_,_,downsampled_relilability = utils.neuronal_CC_paired_stim(higher_odd,lower_odd,neuron_pass=None,to_plot = False,save_dir = os.path.join(save_image,'diff_res_%s_%s'%(higher_res,lower_res)),xlab='odd_%s'%(higher_res),ylab='odd_%s'%(lower_res))


	cc,p = wilcoxon(same_stim_relilability,downsampled_relilability)
	# print(cc,p, p<0.05)
	...
	# diff = parallel_axis_dot(same_stim_relilability,downsampled_relilability,['same_high','same_low'],title='%s,%s'%(higher_res,lower_res),save_image=save_image)
	...
	return '%s,%s,%0.10f'%(higher_res,lower_res,p), diff


wn_files, files_512, files_256, files_128, files_64, files_32 ,sta_files,flash_file = config.get_file_numbers_5res()
spikes_folder = r'F:\Dataset_public\spikes_data\multires_spikes.npy'
all_spikes = np.load(spikes_folder)

save_image = r'F:\Dataset_public\figures\figure_1'
...
for i in range(len(res_set)): 
	for j in range(1,len(res_set) - i):
		...
		# if res_set[i] == to_keep[0] and res_set[i+j] == to_keep[1]:
		# 	isolate_box = diff
...
plt.tight_layout()
fig.savefig(os.path.join(save_image,'all_diff_%d.jpeg'%(40)))
